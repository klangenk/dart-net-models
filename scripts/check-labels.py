...
parser.add_argument('imagePath', type=str, help='path to images')
...
def vconcat_resize_min(im_list):
    ...
    im_list_resize = [
        resize(im, (w_min, int(im.shape[0] * w_min / im.shape[1])))
        for im in im_list
    ]
...
def hconcat_resize_min(im_list):
    ...
    im_list_resize = [
        resize(im, (int(im.shape[1] * h_min / im.shape[0]), h_min))
        for im in im_list
    ]
...
def drawText(img, text, index):
    ...
    pos = ((img.shape[1] - textsize[0]) // 2,
           (img.shape[0] + textsize[1]) // 2 + (index - 1) * 50)
    return cv2.putText(img, text, pos, font, fontScale, color, thickness,
                       cv2.LINE_AA)
...
def square(image):
    ...
    return image[(width - size) // 2:(width - size) // 2 + size,
                 (height - size) // 2:(height - size) // 2 + size, :]
...
folders = sorted([
    f for f in glob(f'{args.imagePath}/*')
    if os.path.isdir(f) and os.path.exists(f"{f}/darts.csv")
    and not os.path.exists(f"{f}/darts_check.csv") and len(glob(f'{f}/*')) > 5
])
    ...
    with open(f'{folder}/darts.csv', newline='') as inFile:
        ...
        with open(f'{folder}/darts_check.csv', 'w', newline='') as outFile:
            ...
            for row in spamreader:
                ...
                imgs = [
                    readImage(file, transforms) for i, file in enumerate(files)
                ]
                img = None
