...
with open('split.csv', newline='') as f:
    reader = csv.reader(f)
    valid = list(reader)
    ...
    for folder in folders:
        ...
        with open(f'{folder}/darts_check.csv', newline='') as csvfile:
            ...
            for row in spamreader:
                [id, *darts] = row
                if id == 'id': continue
                if ([date, id] in valid) == train: continue
                ...
                darts = [d if d != '' else '0-1' for d in darts]
                if all([x == 'empty' or x == '0-1' for x in darts]):
                    darts = ['empty', 'empty', 'empty']
                c = "#".join(darts)
